...
class UndoRequests extends Task
{
    ...
    private function moveRequestToTalk(string $input): void {
        $page = $this->services->newPageGetter()->getFromTitle("نقاش ويكيبيديا:طلبات استرجاع التخريب الكمى");
    ...
    private function init(): void {
        $page = $this->services->newPageGetter()->getFromTitle("ويكيبيديا:طلبات استرجاع التخريب الكمى");
        $text = $page->getRevisions()->getLatest()->getContent()->getData();
                ...
                foreach ($requests as $request){
                    ...
                    foreach ($edits as $edit){
                        if(!$this->revertedCheck($edit["rev_undo"])){
                            ...
                            if($this->services->newRevisionUndoer()->undo($_page->getRevisions()->getLatest(), new EditInfo("بوت استرجاع تخريب كمى: استرجاع تعديلات ${username}"), $undoafter)){
